 ...
 * Underscore prefix tells Vercel this is a helper, not a route.
 */
...
// ── Language Config ─────────────────────────────────────────────
export const LANGUAGE_CONFIGS: Record<string, { elevenlabs: { ttsCode: string | null } }> = {
  lua: { elevenlabs: { ttsCode: null } },
  en: { elevenlabs: { ttsCode: "eng" } },
  fr: { elevenlabs: { ttsCode: "fra" } },
  ln: { elevenlabs: { ttsCode: "lin" } },
  sw: { elevenlabs: { ttsCode: "swa" } },
};
...
// ── Tshiluba Corpus ─────────────────────────────────────────────
const FEW_SHOT: Array<{ en: string; lua: string; fr: string }> = [
...
function buildFewShot(src: string, tgt: string): string {
  return FEW_SHOT
    .map((ex) => {
